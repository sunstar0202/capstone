...
# 🌟 [무적 전용 추가] 태블릿이 '다시 보기'를 눌렀을 때 고유 ID로 이력을 조회하는 엔드포인트
@app.get("/results/{result_id}")
def get_result_by_id(result_id: int):
    for res in results:
        if res.id == result_id:
            print(f"🔄 [이력 재조회 요청] ID: {result_id}번 데이터를 태블릿으로 재전송합니다.")
            return {
                "id": res.id,
                "label": res.label,
                "score": res.score,
                "deviceId": res.deviceId,
                "createdAt": res.createdAt.isoformat()
            }
...
async def predict_trash(file: UploadFile = File(...)):
    try:
        ...
        new_result = AnalysisResultResponse(
            id=len(results) + 1,
            label=label,
            score=encoded_score,
            deviceId="tablet-01",
        ...
        # 실시간 모니터링 터미널 로그
        print("\n" + "="*50)
        print(f"🤖 [AI 실시간 분리수거 분석 리포트]")
        print(f"🆔 분석 번호 : {new_result.id}번 데이터")
        print(f"📦 분류 품목 : {label} (확신도: {confidence.item()*100:.1f}%)")
        print(f"⚠️ 최종 오염도 점수 : {pure_score}%")

        status_1 = "미검출/정상" if c1 == 1 else "이물질/라벨 감지"
        status_2 = "정상" if c2 == 1 else "오염/주의"
        print(f"🔍 세부 코드 상태 : c1={c1}({status_1}), c2={c2}({status_2}), c3={c3}")
        print("="*50 + "\n")
        ...
        return {
            ...
            "deviceId": new_result.deviceId,
            "createdAt": new_result.createdAt.isoformat()
        }
